...
;;(defvar gnats:root "/opt/public/sde/lib/gnats/gnats-db"
;;  "*The top of the tree containing the GNATS database.")

(defvar gnats:libdir (or (gnats::get-config "LIBDIR") "/opt/public/sde/lib")
...
(defvar gnats::version
  (concat "Version " (or (gnats::get-config "VERSION") "3.97")))
...
(put 'gnats::pr-locked 'error-conditions '(error gnats::error gnats::pr-locked))
(put 'gnats::pr-locked 'error-message "PR locked by")
...
Other changes are sent to the responsible person."
  ...
  (let (responsible
	...
	(cond (gnats:::pr-locked
	       ...
	       (message "edit-pr: filing PR...done.")
...
(defun gnats::pr-edit (args &optional min max abuf force)
  (let ((buf (or abuf (get-buffer-create gnats::err-buffer)))
	result)
    (unwind-protect
	(let ((command (append (list 'funcall)
				 (if min (list ''call-process-region min max)
				 (list ''call-process))
			       (list (format 
	    ...
	    (and (/= 0 result)
		 ...
		 (cond
		  ...
		  ((looking-at "n?pr-edit: PR is not locked")
		   (if (not force) (signal 'gnats::pr-not-locked nil)
		     t))
      ...
      (or abuf (kill-buffer buf)))
    (zerop result)))

(defun gnats::check-pr ()
  (let ((err-buffer (get-buffer-create gnats::err-buffer))
	(result nil))
    (unwind-protect
	(condition-case foo
	    (gnats::pr-edit (append (list "--check")
				   (if gnats:network-server
				       (list "--host" gnats:network-server)))
			   (point-min) (point-max) err-buffer)
	  (gnats::invalid-fields
	   (setq gnats:::pr-errors err-buffer)
	   (setq err-buffer nil)))
      (cond (err-buffer (kill-buffer err-buffer))))
...
(defun gnats:unlock-pr-force (pr)
  (gnats::pr-edit (append (list "--unlock" pr)
			 (if gnats:network-server
			     (list "--host" gnats:network-server)))
		 nil nil nil t))
	   ...
	   (let ((pr gnats:::buffer-pr))
		 ...
		 (let ((old-buf (get-buffer
		   ...
		   (cond (old-buf
			  ...
			  (kill-buffer old-buf)))
		   (set-buffer buf)
		   (rename-buffer (concat "*edited-pr " pr "*"))))))
	...
	(if (not buf)
	  ...
	  (save-excursion
	    ...
	    (and (/= 0 result)
		 (goto-char (point-min))
		 (cond
		  ((or (looking-at "n?query-pr: no PRs matched")
		       (looking-at "n?query-pr: couldn.t find PR.*"))
		   (signal 'gnats::no-such-pr nil))
		  (t (signal 'gnats::error
			     (list (buffer-substring (point-min)
						     (- (point-max) 1)))))
...
(defun gnats:edit-pr (&optional id)
  ...
  (interactive "sPR number: ")
  (let (pr category err-buffer temp-name msg newfile)
    (if (string= id "")
	...
	(condition-case var
	  ...
	  ;; Error handler
	  (gnats::pr-locked
	   ;; it's either locked by us or someone else
	   (let ((buf (get-buffer pr)) win)
	     (if buf
		 ...
		 (progn
		   (if (setq win (get-buffer-window buf))
		     ...
		     (switch-to-buffer buf))
		   (message
		    (format "edit-pr: already editing PR %s." pr)))
	       ;; kick it to the next level
	       (signal 'gnats::pr-locked (cdr var))))))
      (signal 'gnats::no-such-pr (list id)))))
...
to find the text that the hits refer to."
  ...
  (compile-internal (concat
		     (if gnats:network-server
			 (format "nquery-pr --host %s " gnats:network-server)
		       "query-pr ")
		     options)
...
(defun gnats::tr (string from to)
  (let ((s (copy-sequence string))
    ...
    (while (>= (setq len (1- len)) 0)
      (if (string-match (regexp-quote (char-to-string (aref s len))) from)
	  (aset s len (aref to (match-beginning 0)))))
    s))
