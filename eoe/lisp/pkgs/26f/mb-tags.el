...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Module Description:
;;; Mohsen's hacks for harris-tags.el
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; File:	harris-tags.el, version 1.09
...
(defconst identifier-boundary-chars "[^A-Za-z_:\-]"
  "Characters used to bound identifiers")
...
(defun old-visit-tags-table (file)
...
Any buffer-local value of tags-file-name for the current buffer cleared."
  ...
  (if (file-directory-p file)
      (setq file (file-name-as-directory file)
	    file (concat file (if (file-exists-p (concat file "TAGS")) "TAGS" "tags"))))
  (setq file (expand-file-name (chase-symlinks file)))
  (setq auto-accept-tags-file-names (list "TAGS" "tags" file))
  (if (eq inhibit-setting-tags-file-name 'global)
      (message "Local tags table now %s" (setq tags-file-name file))
    (or inhibit-setting-tags-file-name
	(progn (message "Default tags table now %s"file)
...
(defun find-tag-default ()
  (save-excursion
	...
	(progn (forward-char 1)
	       (buffer-substring (point)
				 (if (re-search-backward identifier-boundary-chars
							 (point-min)
							 1)
				     (1+ (point))
				   (point))))
...
(defun tags-apropos(string)
  ...
  (interactive (if current-prefix-arg
		   '(nil t)
		 (completing-find-tag-tag "Tags Apropos (regexp): ")))
  (do-tags-apropos string))


(defun completing-get (string)
  "May be this should be added to find-tag-tag"
 (let* ((tag-completion-table (current-tag-completion-table))
	 (beg (save-excursion
		(if (re-search-backward identifier-boundary-chars
					(point-min)
					1)
...
(defun tags-help ()
  "Create a help buffer with a brief description of the mb-tags Capabilities."
  (interactive)
  (with-output-to-temp-buffer "*Help*"
    (princ
     (format
...
ESC SPC		complete-tag
ESC ,		tags-loop-continue
ESC .		completing-find-tag-other-window
C-x 4 .		find-tag-other-window

Tags Related functions are:\n
list-tags		      
  Function: Display list of tags in file FILE.
tags-apropos		      
  Function: 
list-tags		      
  Function: Display list of tags in file FILE.
tags-loop-continue	      ESC ,
  Function: Continue last M-x tags-search or M-x tags-query-replace command.
tags-query-replace	      
  Function: Query-replace-regexp FROM with TO through all files listed in tag table.
tags-search		      
  Function: Search through all files listed in tag table for match for REGEXP.
visit-tags-table	      
  Function: Tell tags commands to use tag table file FILE.
completing-find-tag-other-window
  Function: Find tag (in current tag table) whose name contains TAGNAME.
 	Selects the buffer that the tag is contained in in another window
	and puts point at its definition.
	"
      ))))
