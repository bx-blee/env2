;;; 
;;; RCS: $Id: ndmt-lsm-ua-lucid.el,v 1.1.1.1 2007-02-23 22:32:57 mohsen Exp $
;;;
...
(defun ndmt-lsm-ua-test-insert-submit-new (&optional num-copies)
  (let (buf
    ...
    (while (<= count num-copies)
      ...
      (setq test-message (format "From %s@neda.com %s
Date: %s
From: %s
To: %s
Subject: NDMT originated Test Message %d/%d from host %s
X-Warning: UNAuthenticated Sender
Message-ID: <NDMT.%s.%d@%%s.neda.com>
MIME-Version: 1.0
...
(defun ndmt-lsm-ua-known-ua-users ()
  (let (cur-buf ini-buf curr-id curr-name p1)
    ...
    (while (re-search-forward "^\\[[0-9][0-9][0-9]\\.[0-9][0-9][0-9]\\]" nil t)
      (setq curr-id (buffer-substring (- (point) 8) (- (point) 1)))
...
(defun ndmt-test-message-to-field ()
  (setq *ndmt-test-message-previous-to-field*
	(read-string "Enter `To:' field: "
			 ...
			 (format "(NDMT Test Msg Recipient) <%s@lsm.neda.com>"
				 (ndmt-ua-user-id))))))


(defun ndmt-test-message-from-field ()
  (format "(NDMT Test Msg Originator) <%s@neda.com>"
	  (user-login-name)))  
